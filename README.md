...
## 2022-03-30 진행상황
  ...
  + 2차 피드백 진행
    + 깃허브 쓰는 방식 잘못됨. 협업의 의미가 없다 -> 서로 작성해야 할 폴더를 나눠 그 안에서만 작업을 진행하였고 병합시에도 각자의 브랜치에서 코드를 다운받아 폴더만 추가해 주는 식으로 병합을 진행했었음 -> git을 제대로 활용하지 못함 -> git merge 등의 명령어를 이용해 서로의 코드를 주기적으로 병합해야 했는데 이해를 잘못한채로 진행해옴